...
import SpotCheck2 from '../../src/components/SpotCheck2';
import Adapter from 'enzyme-adapter-react-16';
import { mount, configure} from 'enzyme';

configure({ adapter: new Adapter() });

describe("Spotcheck 2", () => {
    ...
    it('You should define a toggle function on your component, which changes the state of showHome', () => {
      const wrapper = mount(<SpotCheck2/>)
        expect(typeof wrapper.instance().toggle, 'You should define a toggle method on your component')
          .toBe("function")
        let initial = wrapper.state().showHome
        wrapper.instance().toggle()
        expect(wrapper.state().showHome, 'The toggle function should change teh state of showHome from true to false or false to true')
          .toBe(!initial)
    });
